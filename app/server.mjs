...
export function createApplication({ database = resolve(root, '../runtime/lens.sqlite'), apiKey = process.env.OPENAI_API_KEY || '', model = process.env.OPENAI_MODEL || 'gpt-6-astra', fetcher = fetch, clock = Date.now, provider = 'openai', baseUrl, authorize, modelHooks = {} } = {}) {
  ...
  const taskRequest = b => {
    const task_type=b.task_type??'REPLY';
    if(task_type!=='REPLY')return normalizeCustomerTask({task_type,goal:b.goal,text:b.text,materials:b.materials});
    if(b.materials!==undefined)fault(400,'聊天回复请提交经核对的聊天文字；画像资料请使用新客画像任务。');
    const text=field(b.text,20000,true);
    if(text.length<4)fault(400,'请提供至少 4 个字符的上下文。');
    return {task_type,text,goal:choice(b.goal,goals)};
  };
 ...
 const sid = streamerId(b.streamer_id); const pid=field(b.person_id,80,true);if(!/^[a-zA-Z0-9-]+$/.test(pid))fault(400,'人物编号无效。'); const p = store.context(pid,sid); if (!p) fault(404, '请先建立当前主播与此人物的关系档案。');
        const task=taskRequest(b),{text,goal,task_type}=task;
        const mode = choice(b.mode, ['local','model']);
        if(task_type!=='REPLY'&&mode!=='model')fault(400,'新客画像与破冰开场需要联网分析。');
        if(task_type==='PROFILE'&&p.segment!=='NEW')fault(409,'请先将此客户归为新用户，再提交新客画像。');
        ...
        const fingerprint = hash(JSON.stringify({ text, goal, mode, model: mode === 'model' ? config.model : 'rules-v3', contextRevision,
          ...(task_type==='REPLY'?{}:{task_type,materials:task.materials}) }));
        ...
        try {
          const sourceText=task_type==='REPLY'?text:[text,...task.materials.map(m=>m.text)].join('\n');
          const situation = task_type==='REPLY'?classify(text,goal):task_type;
          const pack = packContext(p,sourceText,goal,situation);
          const result = task_type!=='REPLY' ? await customerTaskAnalysis(task,pack.context,config,fetcher)
            : mode === 'model' ? await modelAnalysis(text, pack.context, goal, config, fetcher) : localAnalysis(text, pack.context, goal);
          ...
          result.customer_revision=contextRevision;
          const actualMode = result.judge.source === '独立模型调用' ? 'model' : 'local';
          let belief = task_type==='REPLY'?makeBelief(result):null;
          ...
          assertCurrent();
          return store.saveAnalysis(p, task_type==='REPLY'?text:JSON.stringify(task), goal, fingerprint, result, actualMode, belief);
  ...
  const preflight=b=>{if(active.has(b.person_id))fault(409,'该人物已有分析任务，本次未调用模型，请等待完成。');if(active.size>=2)fault(429,'正在处理其他分析，请稍后重试。');taskRequest(b);choice(b.mode,['local','model']);};
  ...
  const handle = nativeOnly => async (req, res) => {
    ...
    try {
      ...
      if (!path.startsWith('/api/')) {
        ...
        const files = { '/': ['index.html','text/html; charset=utf-8'], '/app.js': ['app.js','text/javascript; charset=utf-8'], '/design-tokens.css': ['design-tokens.css','text/css; charset=utf-8'], '/style.css': ['style.css','text/css; charset=utf-8'], '/icon.svg': ['icon.svg','image/svg+xml'], '/manifest.webmanifest': ['manifest.webmanifest','application/manifest+json'] };
  ...
  return { server, nativeServer, store, native, analyze, revision:contextKey, isActive: id => active.has(id) };
}
