...
export function invalidateDerived(store, ids) {
  ...
  while(changed){changed=false;for(const row of rows){
    ...
    const dependencies=[...(receipt?.outcome_ids||[]),...(receipt?.reviewed_belief_ids||[])];
    if(!dependencies.some(id=>affected.has(id)))continue;
    affected.add(row.id);changed=true;count++;
    store.run('UPDATE analyses SET result=?,fingerprint=? WHERE id=?',JSON.stringify({summary:'引用的历史资料已删除或到期，请重新分析。',route:'SAFE_STOP',strategy:'重新分析',risk:'旧建议已失效',reason:'历史资料变更',evidence:[],candidates:[],judge:{verdict:'BLOCK',source:'资料生命周期校验'},context_receipt:{invalidated:true}}),randomUUID(),row.id);
