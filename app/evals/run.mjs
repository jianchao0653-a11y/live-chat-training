...
try {
  ...
  if(!['local','live','validate'].includes(mode))throw new Error('Invalid mode');
  let modelOptions={};
  // Offline evaluation must not even inspect model credentials or their file paths.
  if(mode==='live') {
    ...
    modelOptions={apiKey,provider,baseUrl:process.env.LENS_BAILIAN_BASE_URL,
      model:process.env.LENS_BAILIAN_MODEL||(provider==='bailian'?'qwen-plus':process.env.OPENAI_MODEL||'gpt-6-astra')};
  }
  const {report,directory}=await evaluateText({mode,manifestPath:options['--manifest'],
    ...(options['--limit']!==undefined?{limit:Number(options['--limit'])}:{}),
    ...modelOptions,outputRoot:fileURLToPath(new URL('../../output/evals/',import.meta.url))});
