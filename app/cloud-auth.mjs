...
export function openCloudAuth(file,clock=Date.now) {
  ...
  db.exec(`PRAGMA foreign_keys=ON; PRAGMA secure_delete=ON; PRAGMA busy_timeout=5000;
    ...
    CREATE TABLE IF NOT EXISTS customer_changes(seq INTEGER PRIMARY KEY AUTOINCREMENT, account TEXT NOT NULL REFERENCES accounts(id), person TEXT NOT NULL, segment TEXT NOT NULL CHECK(segment IN ('UNCLASSIFIED','NEW','MAINTAIN')), created INTEGER NOT NULL);
    CREATE INDEX IF NOT EXISTS customer_change_account ON customer_changes(account,seq);`);
  // Add metadata to the existing spend ledger; never reset historical usage.
  if(!db.prepare('PRAGMA table_info(tasks)').all().some(c=>c.name==='task_type'))
    db.exec("ALTER TABLE tasks ADD COLUMN task_type TEXT NOT NULL DEFAULT 'REPLY'");
  db.exec('CREATE INDEX IF NOT EXISTS task_account_type_date ON tasks(account,task_type,created)');
