...
function renderSettings() {
  $('#main').innerHTML = heading('按你的方式，开始测试。','连接模型，查看能力范围，管理本地数据。') + `<div class="settings-grid"><section class="panel"><h2>模型连接</h2><p>接入后可分析复杂对话，并通过两次独立调用完成生成与终审。未接入时可以体验本地基础规则。</p>
    ...
    <div><section class="panel"><h2>本版能力</h2>${[['电脑 / 手机浏览器工作台','已实现 · 响应式'],['人物、关系与证据数据库','已实现 · SQLite'],['6+2 分析与独立终审','规则可用 / 模型待连接测试'],['截图转写后人工核对','需图片输入模型'],['反馈与贝叶斯记录','已实现 · 实验参数'],['Android 原生输入法与建议','v0.13 模拟器预览'],['单帧授权与确认插入','Android 模拟器已验证'],['iPhone 原生入口','待 macOS / Xcode 实施']].map(([k,v])=>`<div class="capability"><span>${k}</span><small>${v}</small></div>`).join('')}</section>
  ...
  if(state.config.local) {
    $('#main').insertAdjacentHTML('beforeend',`<section class="panel"><h2>Android 设备连接</h2><p>为当前主播 ${esc(s.name)} 配对手机。设备仅能访问这位主播已建立的关系；连接八小时到期，电脑重启服务后需要重新配对。</p><button id="pairDevice" class="primary">生成两分钟配对码</button><p id="pairCode" role="status"></p><div id="deviceList"></div></section>`);
