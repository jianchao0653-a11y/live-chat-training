...
test('lost customer-create response can be retried after restart without duplicate or resurrection',async t=>{
  const f=await fixture(t),account=await f.activate(),other=await f.activate();
  const body={operation_id:randomUUID(),name:'synthetic retry',platform:'微信',segment:'NEW'};
  const first=await f.request('/api/native/library/people',body,account.token);assert.equal(first.status,200);
  // Client discards the first success response, then retries the exact operation.
  await f.restart();
  const again=await f.request('/api/native/library/people',body,account.token);
  assert.equal(again.status,200);assert.equal(again.body.id,first.body.id);
  const roster=await f.request('/api/native/library/people',null,account.token);assert.equal(roster.body.people.length,1);
  assert.equal((await f.request('/api/native/library/people',{...body,name:'changed'},account.token)).status,409);
  const outsider=await f.request('/api/native/library/people',body,other.token);assert.notEqual(outsider.body.id,first.body.id);
  await f.request(`/api/native/library/people/${first.body.id}/delete`,{},account.token);
  assert.equal((await f.request('/api/native/library/people',body,account.token)).status,409);
  assert.equal(f.calls,0);
});

test('same-name roster rows retain their own current relationship notes and account boundary',async t=>{
  const f=await fixture(t),a=await f.activate(),b=await f.activate();
  const first=await f.person(a),second=await f.person(a),outside=await f.person(b);
  for(const [account,person,notes] of [[a,first,'合成背景甲'],[a,second,'合成背景乙'],[b,outside,'另一账号背景']]){
    assert.equal((await f.request(`/api/native/library/people/${person.id}/save`,{
      name:'同名客户',platform:'微信',stage:'初识',notes,boundary:''
    },account.token)).status,200);
  }
  await f.restart();
  const rows=(await f.request('/api/native/library/people',null,a.token)).body.people;
  assert.equal(rows.length,2);
  assert.deepEqual(rows.map(p=>[p.id,p.pair_id,p.notes]).sort(),[
    [first.id,first.relationship.id,'合成背景甲'],[second.id,second.relationship.id,'合成背景乙']
  ].sort());
  assert.ok(rows.every(p=>p.name==='同名客户'&&p.platform==='微信'));
  const theirs=(await f.request('/api/native/library/people',null,b.token)).body.people;
  assert.deepEqual(theirs.map(p=>[p.id,p.notes]),[[outside.id,'另一账号背景']]);
  assert.equal(f.calls,0);
});
