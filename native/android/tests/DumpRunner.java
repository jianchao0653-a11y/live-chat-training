...
public final class DumpRunner extends Instrumentation {
    ...
    @Override public void onStart() {
        ...
        try {
            ...
            try { ui.waitForIdle(500, 3000); }
            catch (TimeoutException stillBusy) { /* bounded current-window read below */ }
            Thread.sleep(250);
            if(arguments.containsKey("hint")){
                boolean changed=false;
                long deadline=android.os.SystemClock.uptimeMillis()+5000;
                while(!changed && android.os.SystemClock.uptimeMillis()<deadline){
                    for(AccessibilityWindowInfo window:ui.getWindows())changed|=setText(window.getRoot());
                    if(!changed)Thread.sleep(100);
                }
