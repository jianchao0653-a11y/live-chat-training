...
public final class LibraryActivity extends Activity {
    ...
    private void page(String title){generation++;ScrollView scroll=new ScrollView(this);scroll.setFillViewport(true);body=new LinearLayout(this);body.setOrientation(LinearLayout.VERTICAL);body.setPadding(LensStyle.dp(this,ActivityLayout.GUTTER_DP),LensStyle.dp(this,ActivityLayout.PADDING_Y_DP),LensStyle.dp(this,ActivityLayout.GUTTER_DP),LensStyle.dp(this,ActivityLayout.PADDING_Y_DP));body.setBackgroundColor(LensStyle.BG);scroll.addView(body);
        ...
        label(title,LensTokens.ACTIVITY_TITLE_SP);status=label("",LensTokens.TEXT_BODY_SP);status.setAccessibilityLiveRegion(android.view.View.ACCESSIBILITY_LIVE_REGION_POLITE);setContentView(scroll);}
    private TextView label(String text,int size){TextView v=new TextView(this);v.setText(text);LensStyle.text(v,size,size>ActivityLayout.LIBRARY_HEADING_ABOVE_SP);body.addView(v,LensStyle.space(this));return v;}
    private EditText field(String hint,String value,int id,boolean multi){EditText v=new EditText(this);v.setId(id);v.setSaveEnabled(false);v.setHint(hint);v.setText(value);v.setInputType(InputType.TYPE_CLASS_TEXT|(multi?InputType.TYPE_TEXT_FLAG_MULTI_LINE:0));if(multi)v.setMinLines(ActivityLayout.LIBRARY_EDITOR_MIN_LINES);LensStyle.field(v);body.addView(v,LensStyle.space(this));return v;}
    ...
    private void login(){page("登录观微");label("输入收到的一次性邀请码。试用期资料保存在项目提供者的电脑后端，网络请求经过 ngrok HTTPS 网关，网关可处理请求内容。只有你批准的聊天片段与必要人物背景会提交给阿里云百炼分析。建议由你编辑、确认并发送。",LensTokens.TEXT_SUBHEADING_SP);
        label("分析与反馈保留期30天；人物与确认记忆保留至删除。备份保留期7天，服务运行时每小时清理，停机后恢复时清理。可在人物资料中修改和删除；卸载应用不会删除云端资料。",LensTokens.TEXT_BODY_SP);
    ...
    private void profile(){page("我的主播档案");button("返回",()->home());call("library/streamer",null,r->{JSONObject p=r.getJSONObject("profile");
        label("当前账号："+r.getString("account_id")+"\n这些资料由你填写，只用于当前账号的回复建议。",LensTokens.TEXT_BODY_SP);
    ...
    private void home(){page("人物与聊天记忆");button("我的主播档案",()->profile());button("新建人物",()->editPerson(null));button("返回",()->finish());
        ...
        call("library/people",null,r->{status.setText("选择人物查看、修改或删除资料。离线时仍可正常打字。");JSONArray list=r.getJSONArray("people");if(list.length()==0)label("还没有人物，先填写一个昵称即可开始。",LensTokens.TEXT_HEADING_SP);for(int i=0;i<list.length();i++){JSONObject p=list.getJSONObject(i);button(p.getString("name")+" · "+p.getString("platform"),()->detail(p.optString("id")));}});
    }
    private void editPerson(JSONObject p){final String operationId=java.util.UUID.randomUUID().toString();page(p==null?"新建人物":"修改人物");EditText name=field("昵称",p==null?"":p.optString("name"),3102,false);label("聊天平台",LensTokens.TEXT_SUPPORT_SP);Spinner platform=options(new String[]{"微信","抖音","快手","视频号"},p==null?"微信":p.optString("platform"));label("关系阶段",LensTokens.TEXT_SUPPORT_SP);Spinner stage=options(new String[]{"初识","熟悉中","稳定联系","需要修复"},p==null?"初识":p.optString("stage"));
    ...
    private void detail(String id){page("人物资料");button("返回人物列表",()->home());call("library/people/"+id,null,p->{status.setText(p.getString("name")+" · "+p.getString("platform"));label("关系："+p.optString("stage")+"\n背景："+p.optString("notes")+"\n边界："+p.optString("boundary"),LensTokens.TEXT_SUBHEADING_SP);
        ...
        JSONArray claims=p.getJSONArray("claims");label("记忆与标记",LensTokens.ACTIVITY_SECTION_SP);for(int i=0;i<claims.length();i++){JSONObject c=claims.getJSONObject(i);label(("TAG".equals(c.optString("category"))?"标记 · ":"记忆 · ")+kindLabel(c.optString("kind"))+" · "+("PENDING".equals(c.optString("review_state"))?"待确认":"已确认状态")+"\n"+c.optString("content")+"\n来源："+c.optString("source")+"\n"+c.optString("created_at"),LensTokens.TEXT_SUBHEADING_SP);button("修改或删除",()->memory(id,c));if("PENDING".equals(c.optString("review_state")))button("确认此条推测",()->confirm("确认后可作为推测参考，仍保留推测来源；旧分析与反馈会清除。",()->{try{call("library/memories/"+c.optString("id")+"/confirm",new JSONObject().put("confirmed",true),v->{AssistSession.clear();detail(id);});}catch(Exception e){status.setText("确认失败。");}}));}
    });}
    ...
    private void memory(String id,JSONObject c){page(c==null?"添加记忆或标记":"修改记忆或标记");label("请选择真实来源。推测保存后需单独确认，修改推测后也需要重新确认。",LensTokens.TEXT_BODY_SP);Spinner kind=options(KIND_LABELS,kindLabel(c==null?"SELF_DECLARED":c.optString("kind"))),category=options(new String[]{"聊天记忆","客户标记"},c!=null&&"TAG".equals(c.optString("category"))?"客户标记":"聊天记忆");EditText content=field("内容",c==null?"":c.optString("content"),3105,true),source=field("来源，例如：哪次聊天中的原话",c==null?"":c.optString("source"),3106,true);
    ...
    private void history(String id){page("分析与反馈");button("返回人物资料",()->detail(id));call("library/people/"+id+"/history",null,r->{status.setText("最近30条记录；插入不代表已经发送或得到积极回应。");JSONArray list=r.getJSONArray("history");for(int i=0;i<list.length();i++){JSONObject a=list.getJSONObject(i);label(a.optString("created_at")+" · "+a.optString("goal")+"\n"+a.optString("summary"),LensTokens.TEXT_SUBHEADING_SP);JSONObject o=a.optJSONObject("outcome");if(o!=null)label("反馈："+o.optString("status")+"\n"+o.optString("note"),LensTokens.TEXT_BODY_SP);button("填写或修改反馈",()->feedback(id,a));button("删除反馈及旧分析",()->confirm("清除此人物的旧分析与反馈，避免已删除的反馈残留在其他分析中。",()->call("library/feedback/"+a.optString("id")+"/delete",new JSONObject(),v->detail(id))));}});}
