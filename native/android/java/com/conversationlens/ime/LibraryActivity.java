...
public final class LibraryActivity extends Activity {
    ...
    private void page(String title){generation++;ScrollView scroll=new ScrollView(this);scroll.setFillViewport(true);body=new LinearLayout(this);body.setOrientation(LinearLayout.VERTICAL);body.setPadding(20,24,20,24);body.setBackgroundColor(LensStyle.BG);scroll.addView(body);
    ...
    private void detail(String id){page("人物资料");button("返回人物列表",()->home());call("library/people/"+id,null,p->{status.setText(p.getString("name")+" · "+p.getString("platform"));label("关系："+p.optString("stage")+"\n背景："+p.optString("notes")+"\n边界："+p.optString("boundary"),15);
        button("修改人物与关系",()->editPerson(p));button("添加确认记忆",()->memory(id,null));button("查看分析与反馈",()->history(id));
        ...
        JSONArray claims=p.getJSONArray("claims");label("确认记忆",20);for(int i=0;i<claims.length();i++){JSONObject c=claims.getJSONObject(i);label(c.optString("content")+"\n来源："+c.optString("source")+"\n"+c.optString("created_at"),15);button("修改或删除这条记忆",()->memory(id,c));}
    });}
    private void memory(String id,JSONObject c){page(c==null?"添加记忆":"修改记忆");label("只记录你确认的事实。推测不要登记为事实。",14);EditText content=field("记忆内容",c==null?"":c.optString("content"),3105,true),source=field("来源，例如：本人明确说过",c==null?"":c.optString("source"),3106,true);
        button("保存记忆",()->{try{JSONObject b=new JSONObject().put("content",value(content)).put("source",value(source));Runnable save=()->call(c==null?"library/people/"+id+"/memories":"library/memories/"+c.optString("id")+"/save",b,r->detail(id));if(c==null)save.run();else confirm("修改记忆会清除该人物旧分析与反馈，避免旧内容再次被引用。",save);}catch(Exception e){status.setText("请填写内容和来源。");}});
