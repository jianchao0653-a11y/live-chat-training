...
public final class AssistantActivity extends Activity {
    ...
    private static final class TransientForm {
        String text, personId; int goal, mode; boolean ocrSource;
    }
    @Override public void onCreate(Bundle state){
        ...
        ScrollView scroll=new ScrollView(this);body=new LinearLayout(this);body.setOrientation(LinearLayout.VERTICAL);body.setPadding(LensStyle.dp(this,ActivityLayout.GUTTER_DP),LensStyle.dp(this,ActivityLayout.GUTTER_DP),LensStyle.dp(this,ActivityLayout.GUTTER_DP),LensStyle.dp(this,ActivityLayout.PADDING_Y_DP));body.setBackgroundColor(LensStyle.BG);scroll.setFillViewport(true);scroll.addView(body);
        ...
        label("观微 · 聊天建议",LensTokens.ACTIVITY_TITLE_SP);label("仅分析你核对并批准的片段。返回聊天后再次确认人物，候选只插入输入框，发送由你完成。",LensTokens.TEXT_SUBHEADING_SP);
        status=label("请连接服务，再准备聊天片段",LensTokens.TEXT_BODY_SP);status.setAccessibilityLiveRegion(View.ACCESSIBILITY_LIVE_REGION_POLITE);
        ...
        feedbackControls();
        if(session==null || !session.alive()){label("连接后，请回到聊天输入框，点击键盘上的“建议”。",LensTokens.TEXT_HEADING_SP);setContentView(scroll);return;}
        label("原输入应用："+session.host,LensTokens.TEXT_SUPPORT_SP);
        LensStyle.section(body,"01  核对人物与目标");
        label("人物",LensTokens.TEXT_SUPPORT_SP);people=spinner(new String[]{"正在加载人物"});label("本次目标",LensTokens.TEXT_SUPPORT_SP);goal=spinner(new String[]{"自然接话","关心近况","修复误会","表达边界"});label("分析方式",LensTokens.TEXT_SUPPORT_SP);mode=spinner(cloud?new String[]{"联网聊天建议"}:new String[]{"本机服务规则试算","GPT 分析（需电脑配置）"});
        LensStyle.section(body,"02  准备聊天片段");
        transcript=edit("聊天片段（可粘贴、输入或从截图提取）",2103,true);
        label("截图在本机识别，左侧为对方、右侧为我。请校对文字并去除标题、时间等内容；受保护的黑屏请改用粘贴。",LensTokens.TEXT_BODY_SP);
        ...
        approved=new CheckBox(this);approved.setText("已核对人物和片段，同意提交给连接的服务");approved.setTextColor(LensStyle.INK);approved.setTextSize(LensTokens.ACTIVITY_CHECKBOX_TEXT_SP);approved.setMinHeight(LensStyle.dp(this,LensTokens.CHECKBOX_MIN_HEIGHT_DP));approved.setButtonTintList(android.content.res.ColorStateList.valueOf(LensStyle.GREEN));body.addView(approved,LensStyle.space(this));
        ...
        TextWatcher watcher=new TextWatcher(){public void beforeTextChanged(CharSequence s,int a,int c,int f){}public void onTextChanged(CharSequence s,int a,int b,int c){ocrSource=false;changed();}public void afterTextChanged(Editable e){}};
        ...
        if(restored!=null){setting=true;transcript.setText(restored.text);ocrSource=restored.ocrSource;goal.setSelection(restored.goal);mode.setSelection(restored.mode);setting=false;status.setText("已保留片段；界面变化后的旧候选失效，请重新批准分析");}
        setContentView(scroll);
    }
    @Override public Object onRetainNonConfigurationInstance(){
        if(transcript==null||session==null||client==null)return null;TransientForm form=new TransientForm();form.text=transcript.getText().toString();form.ocrSource=ocrSource;form.goal=goal.getSelectedItemPosition();form.mode=mode.getSelectedItemPosition();int index=people.getSelectedItemPosition();form.personId=index>=0&&index<roster.length()?roster.optJSONObject(index).optString("id"):"";return form;
    }
    ...
    private void analyze(){
        ...
        if(ocrSource&&OcrReview.hasUnresolvedMarkers(transcript.getText().toString())){status.setText("请先校对识别文字并去除非聊天内容，再批准分析。");return;}
    ...
    private void showResult(JSONObject response){
        ...
        TextView summary=new TextView(this);summary.setText(response.optString("summary"));LensStyle.text(summary,LensTokens.TEXT_SUBHEADING_SP,false);results.addView(summary,LensStyle.space(this));
        ...
        draft=new EditText(this);draft.setId(2104);draft.setSaveEnabled(false);draft.setInputType(InputType.TYPE_CLASS_TEXT|InputType.TYPE_TEXT_FLAG_MULTI_LINE);draft.setHint("选择候选后可修改");LensStyle.field(draft);draft.setMinLines(ActivityLayout.EDITOR_MIN_LINES);
    ...
    private void showImage(){
        ...
        ImageView preview=new ImageView(this);preview.setImageBitmap(session.image);preview.setAdjustViewBounds(true);preview.setMaxHeight(ActivityLayout.IMAGE_PREVIEW_MAX_PX);preview.setContentDescription("待批准的截图预览");imagePanel.addView(preview);
    ...
    private void extractImage(){
        ...
        try{LocalChatOcr.recognize(s.image,new LocalChatOcr.Done(){
            public void success(String text){if(isDestroyed()||token!=work||!s.alive()||s.revision!=revision)return;
                ...
                transcript.setText(text);ocrSource=true;status.setText("识别完成：左侧对方、右侧我。请校对文字；尚未批准分析。");reviewOcr(text);}
    ...
    android.app.AlertDialog reviewOcr(String raw){
        ...
        ScrollView scroll=new ScrollView(this);LinearLayout panel=new LinearLayout(this);panel.setOrientation(LinearLayout.VERTICAL);panel.setPadding(LensTokens.OCR_PANEL_PADDING_X_PX,LensTokens.OCR_PANEL_PADDING_Y_PX,LensTokens.OCR_PANEL_PADDING_X_PX,LensTokens.OCR_PANEL_PADDING_Y_PX);scroll.addView(panel);
        TextView help=new TextView(this);help.setText("左侧为对方、右侧为我，已按位置标好。请纠正错字；标题、时间和系统提示取消纳入。未定位到聊天气泡的内容默认不纳入，取消不会回填修改。");LensStyle.text(help,LensTokens.TEXT_BODY_SP,false);panel.addView(help);
        for(int i=0;i<lines.size();i++){
            final OcrReview.Line line=lines.get(i);
            TextView role=new TextView(this);role.setId(24000+i);role.setText(line.speaker==1?"我 · 右侧":line.speaker==2?"对方 · 左侧":"非聊天 · 未定位聊天气泡");role.setContentDescription("第"+(i+1)+"条固定角色："+role.getText());LensStyle.text(role,LensTokens.TEXT_SUBHEADING_SP,true);panel.addView(role,LensStyle.space(this));
            TextView label=new TextView(this);label.setText("第"+(i+1)+"条 · 原始识别："+line.raw);LensStyle.text(label,LensTokens.TEXT_SUPPORT_SP,false);panel.addView(label,LensStyle.space(this));
            EditText text=new EditText(this);text.setId(22000+i);text.setSaveEnabled(false);text.setHint("第"+(i+1)+"条校对文字");text.setContentDescription("第"+(i+1)+"条校对文字");text.setText(line.text);text.setInputType(InputType.TYPE_CLASS_TEXT|InputType.TYPE_TEXT_FLAG_MULTI_LINE);LensStyle.field(text);panel.addView(text,LensStyle.space(this));
            ...
            CheckBox include=new CheckBox(this);include.setId(23000+i);include.setSaveEnabled(false);include.setContentDescription("第"+(i+1)+"条纳入分析");include.setText(line.included?"纳入此条":"不纳入此条");include.setTextColor(LensStyle.INK);include.setTextSize(LensTokens.ACTIVITY_CHECKBOX_TEXT_SP);include.setMinHeight(LensStyle.dp(this,LensTokens.CHECKBOX_MIN_HEIGHT_DP));include.setButtonTintList(android.content.res.ColorStateList.valueOf(LensStyle.GREEN));include.setChecked(line.included);include.setEnabled(line.speaker==1||line.speaker==2);panel.addView(include,LensStyle.space(this));
            include.setOnCheckedChangeListener((button,checked)->{line.included=checked&&(line.speaker==1||line.speaker==2);include.setText(line.included?"纳入此条":"不纳入此条");});
        }
        TextView error=new TextView(this);LensStyle.text(error,LensTokens.TEXT_BODY_SP,false);error.setAccessibilityLiveRegion(View.ACCESSIBILITY_LIVE_REGION_POLITE);panel.addView(error);
    ...
    private void feedbackControls(){
        ...
        label("上次插入 · "+AssistSession.feedbackName+" · 只记录实际后续，不把插入等同于发送或好评",LensTokens.TEXT_BODY_SP);
    ...
    private TextView label(String value,int size){TextView t=new TextView(this);t.setText(value);LensStyle.text(t,size,size>=ActivityLayout.ASSISTANT_HEADING_FROM_SP);body.addView(t,LensStyle.space(this));return t;}
    private EditText edit(String hint,int id,boolean multi){EditText e=new EditText(this);e.setHint(hint);e.setId(id);e.setSaveEnabled(false);e.setInputType(InputType.TYPE_CLASS_TEXT|(multi?InputType.TYPE_TEXT_FLAG_MULTI_LINE:0));if(multi)e.setMinLines(ActivityLayout.EDITOR_MIN_LINES);LensStyle.field(e);body.addView(e,LensStyle.space(this));return e;}
    private Spinner spinner(String[] names){Spinner s=new Spinner(this);s.setAdapter(new ArrayAdapter<String>(this,android.R.layout.simple_spinner_dropdown_item,names));s.setMinimumHeight(LensStyle.dp(this,LensTokens.ACTIVITY_SELECTOR_MIN_HEIGHT_DP));s.setPadding(LensStyle.dp(this,LensTokens.ACTIVITY_SELECTOR_PADDING_X_DP),0,LensStyle.dp(this,LensTokens.ACTIVITY_SELECTOR_PADDING_X_DP),0);s.setBackgroundTintList(android.content.res.ColorStateList.valueOf(LensStyle.GREEN));body.addView(s,LensStyle.space(this));return s;}
