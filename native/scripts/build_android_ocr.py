...
def main():
    ...
    # Fail before native/Gradle work when first-batch design values drift.
    subprocess.run([__import__('sys').executable, str(ROOT/'native/scripts/design_tokens_check.py')], check=True)
    ...
    destination=OUT/('conversation-lens-0.18.0-cloud-unsigned.apk' if args.release else 'conversation-lens-0.18.0-'+('cloud-' if args.cloud_url else '')+'debug.apk')
    shutil.copy2(files[0],destination)
    receipt={'apk':destination.name,'sha256':hashlib.file_digest(destination.open('rb'),'sha256').hexdigest(),'cloudEndpoint':args.cloud_url,'abis':args.abis,'releaseSigned':False,'version':'0.18.0','versionCode':23}
