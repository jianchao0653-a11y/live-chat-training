...
class AcceptanceTest(unittest.TestCase):
    def run_probe(self, health,expected_version=None):
        ...
        with patch('https_acceptance.urllib.request.build_opener') as factory:
            ...
            return probe('https://example.com',expected_version=expected_version)

    def test_healthy_api(self):
        result = self.run_probe(Response(200, b'{"status":"ok","version":"0.18.0"}'))
        self.assertTrue(result['passed'])
        self.assertFalse(result['productionReady'])
        self.assertFalse(result['phoneNetworkVerified'])

    def test_previous_backend_does_not_validate_new_keyboard_release(self):
        result=self.run_probe(Response(200,b'{"status":"ok","version":"0.17.3"}'))
        self.assertFalse(result['passed'])
        self.assertEqual(result['expectedBackendVersion'],'0.18.0')
        self.assertEqual(result['checks'][0]['observedVersion'],'0.17.3')
        old=self.run_probe(Response(200,b'{"status":"ok","version":"0.17.3"}'),'0.17.3')
        self.assertTrue(old['passed'])
        self.assertEqual(old['expectedBackendVersion'],'0.17.3')
