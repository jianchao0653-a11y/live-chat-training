...
if receipt['apk']!='conversation-lens-0.18.0-cloud-unsigned.apk': raise SystemExit('Unexpected prepared artifact')
if receipt.get('version')!='0.18.0' or receipt.get('versionCode')!=23: raise SystemExit('Prepared Android version mismatch')
...
target=OUT/'conversation-lens-0.18.0-cloud-release.apk'
...
receipt.update(apk=target.name,sha256=hashlib.file_digest(target.open('rb'),'sha256').hexdigest(),certificateSHA256=expected,releaseSigned=True,qualityAccepted=False,productionReady=False)
