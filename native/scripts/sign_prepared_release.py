...
if receipt['apk']!='conversation-lens-0.17.1-cloud-unsigned.apk': raise SystemExit('Unexpected prepared artifact')
...
target=OUT/'conversation-lens-0.17.1-cloud-release.apk'
...
receipt.update(apk=target.name,sha256=hashlib.file_digest(target.open('rb'),'sha256').hexdigest(),certificateSHA256=expected,releaseSigned=True,version='0.17.1',versionCode=19,qualityAccepted=False,productionReady=False)
