...
def paired_status(config=None):
    config=config if config is not None else json.loads((OUT/'fixture.json').read_text(encoding='utf-8'))
    name=config['streamer']['name']
    if not isinstance(name,str) or not name:raise AssertionError('Synthetic fixture streamer name missing')
    return '已连接 · 主播 '+name
...
def pair():
    ...
    code=api('devices/pairing','POST',{'streamer_id':config['streamer']['id']})['code']
    fill('电脑设置页的六位配对码',code)
    tap_text('配对连接')
    wait_labels(lambda values:paired_status(config) in values)
    print('PASS paired_scoped_roster',flush=True)

def analyze():
    fill('聊天片段（可粘贴、输入或从截图提取）','对方：今天加班很累，想安静休息。')
