...
const streamer=boot.streamers.find(value=>value.id==='0001');
if(!streamer || typeof streamer.name!=='string' || !streamer.name)throw new Error('Synthetic fixture streamer missing');
...
writeFileSync(new URL('../../output/native/fixture.json',import.meta.url),JSON.stringify({pid:process.pid,base,csrf:boot.csrf,streamer,person,pairing}));
